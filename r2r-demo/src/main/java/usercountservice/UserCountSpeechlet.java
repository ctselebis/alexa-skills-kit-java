...
public class UserCountSpeechlet implements Speechlet {
    ...
    public SpeechletResponse onIntent(final IntentRequest request, final Session session)
        ...
        } else if ("AddNewLayout".equals(intentName)) {
	        try {

		        Map<String, Slot> slots = intent.getSlots();

		        // Get the color slot from the list of slots.
		        Slot nameSlot = slots.get("Layout");
		        return getAddNewLayoutResponse(nameSlot.getValue());
	        }
	        catch (IOException e) {
		        throw new RuntimeException(
			        "Cannot add new page to Liferay", e);
	        }
        } else if ("DefineKewl".equals(intentName)) {
			return getKewlDefinition();
		}
        else {
    ...
    private SpeechletResponse getUserCountResponse() throws IOException{
	    ...
	    try {
		    HttpGet httpGet = new HttpGet("https://cloud-10-0-20-51.liferay.com/api/jsonws/user/get-company-users-count?companyId=20116");
	 ...
	 * @return SpeechletResponse spoken and visual response for the given intent
	 */
	private SpeechletResponse getAddNewLayoutResponse(String layoutName) throws IOException{
		String speechText = "RESPONSE GOES HERE";
		...
		try {
			HttpGet httpGet = new HttpGet("https://cloud-10-0-20-51.liferay.com/api/jsonws/layout/add-layout/group-id/20147/private-layout/false/parent-layout-id/0/name/"+layoutName+"/-title/-description/type/portlet/hidden/false/-friendly-url");
			httpGet.addHeader(BasicScheme.authenticate(creds, "US-ASCII", false));
			CloseableHttpResponse response = httpclient.execute(httpGet);
			try {
				log.info("NEW LAYOUT: " + layoutName);
				speechText = "The page " + layoutName + " was added.";
			}
			finally {
				response.close();
			}
			...
			speechText = "I'm sorry, " + layoutName + " could not be added.";
		}
		finally {
			httpclient.close();
		}
		...
		// Create the Simple card content.
		SimpleCard card = new SimpleCard();
		card.setTitle("AddNewLayoutIntent");
		card.setContent(speechText);
     ...
     * @return SpeechletResponse spoken and visual response for the given intent
     */
    private SpeechletResponse getKewlDefinition() {
        String speechText = "Cool is a California technical term spelled K-E-W-L.";

        // Create the Simple card content.
        SimpleCard card = new SimpleCard();
        card.setTitle("WhatIsKewl");
        card.setContent(speechText);
